...
def filter_items_property(all_items):
    ...
    for item in all_items:
        if item['fields']['status']['name'].lower() != 'closed' and item['fields']['summary'].find("Unknown Work") < 0:
            ...
            dic_property['target_completion'] = item['fields']['customfield_19015'] if 'customfield_19015' in item['fields'] else ''
...
def generic_file_index(tasks):
    ...
    index.append("DevCompletionDate")
    return index
...
def read_item_info_into_data_frame(items):
    ...
    for item in items:
        if len(item['sub_tasks']) > 0:
            # if item with sub tasks don't need to count estimated time, just estimate sub task
            continue
        if item['parent_key']:
...
def read_item_target_complete_date_into_data_frame(items):
    print("Read Target Completion Date into data frame")
    for item in items:
        if item['target_completion']:
            df.at['DevCompletionDate', item['parent_key']] = "'" + item['target_completion']
...
if __name__ == "__main__":
    ...
    read_item_target_complete_date_into_data_frame(items_info)
